...
export async function POST(request: NextRequest) {
  try {
    ...
    const ipHash = ip ? createHash("sha256").update(`50kmbrig:${ip}`).digest("hex").slice(0, 32) : null;
    ...
    if (dbFehler) {
      ...
      return NextResponse.json({ error: "Die Anmeldung kam nicht durch." }, { status: 500 });
    }
    ...
    /* Der Mailversand darf die Anmeldung nicht mehr umwerfen: gespeichert ist
