...
export async function POST(request: NextRequest) {
  ...
  try {
    const { offerId, type } = await request.json()

    if (!offerId || !type || !['anzahlung', 'schlussrechnung'].includes(type)) {
      return NextResponse.json({ error: 'offerId und type (anzahlung/schlussrechnung) erforderlich' }, { status: 400 })
    }
    ...
    const label = type === 'anzahlung' ? 'Anzahlung' : 'Schlussrechnung'
    ...
    if (existing && existing.length > 0) {
      return NextResponse.json({ error: `${label} wurde bereits erstellt` }, { status: 400 })
    }
    ...
    const titleSuffix = type === 'anzahlung' ? 'Anzahlung 50%' : 'Schlussrechnung 50%'
    const descPrefix = type === 'anzahlung' ? 'Erste Teilrechnung' : 'Zweite Teilrechnung'
    const notes = type === 'anzahlung'
      ? 'Zweite Rechnung (50%) folgt bei Projektabschluss.'
      : 'Erstellt bei Projektabschluss.'
    ...
    const { data: invoice, error: invoiceError } = await supabaseAdmin
      ...
      .insert({
        ...
        offer_id: offer.id,
        title: `${offer.title} – ${titleSuffix}`,
        description: `${descPrefix} (50%) gemäss Offerte "${offer.title}"`,
        status: 'draft',
    ...
    const emailIntro = type === 'anzahlung'
      ? `Anbei die erste Teilrechnung (50% Anzahlung) für <strong>${offer.title}</strong>.`
      : `Das Projekt <strong>${offer.title}</strong> wurde abgeschlossen. Anbei die Schlussrechnung (50%).`

    const emailOutro = type === 'anzahlung'
      ? '<p style="color: #666; font-size: 14px;">Die zweite Teilrechnung (50%) folgt bei Projektabschluss.</p>'
      : ''

    const closingLine = type === 'schlussrechnung'
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1a1a1a; max-width: 600px; margin: 0 auto; padding: 20px;">
            ...
            <div style="background: #f5f5f5; border-radius: 12px; padding: 24px; margin: 24px 0;">
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                  <td style="padding: 8px 0; color: #666;">Betrag (50% ${label})</td>
...
${type === 'anzahlung' ? `Anbei die erste Teilrechnung (50% Anzahlung) für ${offer.title}.` : `Das Projekt ${offer.title} wurde abgeschlossen. Anbei die Schlussrechnung (50%).`}

Rechnungsnummer: ${invoice.invoice_number}
Betrag (50% ${label}): ${formatAmount(invoice.total_amount)}
Fällig am: ${formatDate(invoice.due_date)}

${type === 'anzahlung' ? 'Die zweite Teilrechnung (50%) folgt bei Projektabschluss.\n' : ''}
...
Pierre
    ...
    console.error('Create partial invoice error:', err)
