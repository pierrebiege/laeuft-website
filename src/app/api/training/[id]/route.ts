...
export async function GET(
  ...
  const { data, error } = await supabaseAdmin
    .from('training_plans')
    .select('*, client:clients(*), weeks:training_weeks(*, sessions:training_sessions(*, completion:training_completions(*)))')
    .eq('id', id)
...
export async function PATCH(
  ...
  const allowedFields = ['title', 'status', 'start_date', 'sent_at']
