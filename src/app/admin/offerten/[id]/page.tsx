...
export default function EditOfferPage() {
  ...
  async function loadData() {
    const [clientsRes, servicesRes, offerRes] = await Promise.all([
    ...
    const offerItems = (offer.items || [])
      ...
      .map((item: { id: string; title: string; description: string | null; amount: number }) => ({
        id: item.id,
        title: item.title,
  ...
  if (notFound) {
    return (
      <div className="text-center py-12">
        <p className="text-zinc-500 mb-4">Offerte nicht gefunden</p>
        <Link href="/admin" className="text-zinc-900 dark:text-white hover:underline">
    ...
    return (
      <div className="text-center py-12">
        <p className="text-zinc-500 mb-4">
          Nur Offerten im Entwurf-Status können bearbeitet werden
        </p>
        <Link href="/admin" className="text-zinc-900 dark:text-white hover:underline">
          Zurück zur Übersicht
        </Link>
